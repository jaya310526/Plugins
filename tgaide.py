...
async def aide(message: Message):
    ...
    ©当前版本：{version}
    🧑‍💻<b>[作者](tg://user?id=5405984571)</b> 👪<b>[群组](https://t.me/tgaide)</b> 🗣<b>[频道](https://t.me/tgaide_channel)</b>
    """
    submenu_texts = {
        "翻译": """
    🗺️翻译主菜单🗺️
    【<code>,fyall</code>】<i>开关全局翻译</i>
    【<code>,fyit</code>】<i>独立群/人开关，可以独立设置是否在此群或人翻译（仅限于全局翻译关闭的情况下，如果全局翻译开启则无效）</i>
    【<code>,fyset <源语言> <目标语言></code>】<i>设置翻译语言</i> 
    【<code>,fy <源语言></code>】<i>翻译文本</i> 
    """,
        "索敌": """
    🖕索敌主菜单🖕
    【<code>,sd <on/off></code>】<i>回复信息启动索敌，on为开，off为关</i>
    """,
        "系统": """
    ⚙系统主菜单⚙
    【<code>,aideup</code>】<i>手动更新版本</i>
    【<code>,sent</code>】<i>发送文本(支持md/html)</i>
    """,
        "助手": """
    🛠助手主菜单🛠
    【<code>,admins <留言></code>】<i>@当前群组全部管理员（修改后将不显示管理员名称，并美化）</i>
    【<code>,dmy <条数></code>】<i>删除自身信息（编辑后删除）</i>
    【<code>,info <模式></code>】<i>查询具体信息，all为全部，me为自身，u为回复对象（使用此命令回复一个人的信息），不填模式为当前群组信息</i>
    【<code>,usdtset <地址></code>】<i>usdt地址设置</i>
    【<code>,jy <金额></code>】<i>便携交易发起，使用时请回复某个人的信息</i>
    【<code>,debug</code>】<i>查看pag全部变量和值（包括其他插件和pag本身，排查用）</i>
    """,
        "时间": """
    🕘动态时间主菜单🕘
    <b>本功能开启后会定时修改副名称</b>
    【<code>,aidetime <参数></code>】<i>参数为name则开关此功能，参数为set则设置在线或离线（在线状态会使永久在线功能运行，即使离线也会显示在线），参数为mode则切换随机文本或状态模式</i>
    【<code>,timetxt <文本></code>】<i>随机文本模式配置，不输入参数则返回当前设置的文本列表，输入参数则覆盖设置文本列表，以“-”号为分隔符</i>
    """,
        "api": """
    📡API服务主菜单📡
    【<code>,http <模式> [参数]</code>】<i>第一个参数如果是start或stop则是操作web服务的开关，如果参数为port需要再接第二个参数设置端口号，如果参数为key也需要接入第二个参数设置秘钥</i>
    API使用教程：
    使用<code>,http start</code>开启api服务
    访问：http://【你的IP地址】:【你设置的端口号（默认为6868）】/?key=【你设置的秘钥（默认为admin）】&mode=【模式选择，目前只有发送，填写数字1】&id=【发送的id】&msg=【要发送的信息，支持html与md】
    """
    }
    if submenu_name:
